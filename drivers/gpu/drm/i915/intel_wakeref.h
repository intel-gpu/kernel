...
struct intel_wakeref_ops {
	...
	int (*tryget)(struct intel_wakeref *wf);
...
int __intel_wakeref_tryget_first(struct intel_wakeref *wf);
...
static inline int
intel_wakeref_tryget(struct intel_wakeref *wf)
{
	might_sleep();
	if (unlikely(!atomic_inc_not_zero(&wf->count)))
		return __intel_wakeref_tryget_first(wf);
	return 0;
}
