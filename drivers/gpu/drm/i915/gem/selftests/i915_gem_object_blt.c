...
#define WINDOW_BLT_COPY_RETRY		3
	...
	if (err) {
		pr_err("i915_blt_eviction failed. %d\n", err);
		goto err_unpin_dst;
	}
...
static int igt_obj_window_blt_copy(void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(size); i++) {
		...
		if (ret < 0) {
			pr_err("%s: Failed at lmem->lmem size: %llu\n",
			       __func__, size[i]);
			break;
		}
		ret =  __igt_obj_window_blt_copy(i915, smem, smem, size[i]);
		if (ret < 0) {
			pr_err("%s: Failed at smem->smem size: %llu\n",
			       __func__, size[i]);
			break;
		}
		ret =  __igt_obj_window_blt_copy(i915, lmem, smem, size[i]);
		if (ret < 0) {
			pr_err("%s: Failed at lmem->smem size: %llu\n",
			       __func__, size[i]);
			break;
		}
		...
		if (ret < 0) {
			pr_err("%s: Failed at smem->lmem size: %llu\n",
			       __func__, size[i]);
			break;
		}
	}
