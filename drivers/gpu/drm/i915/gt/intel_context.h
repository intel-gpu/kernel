...
int intel_context_try_enter_engine(struct intel_context *ce);
...
static inline int intel_context_try_enter(struct intel_context *ce)
{
	lockdep_assert_held(&ce->timeline->mutex);
	if (!ce->active_count++)
		if (ce->ops->try_enter(ce)) {
			ce->active_count--;
			return -EBUSY;
		}
	return 0;
...
static inline struct intel_timeline *__must_check
intel_context_timeline_trylock(struct intel_context *ce)
	__acquires(&ce->timeline->mutex)
{
	struct intel_timeline *tl = ce->timeline;
...
struct i915_request *intel_context_try_create_request(struct intel_context *ce);
