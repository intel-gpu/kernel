...
cacheline_alloc(struct intel_timeline_hwsp *hwsp, unsigned int cacheline)
{
	...
	type = i915_coherent_map_type(hwsp->gt->i915, hwsp->vma->obj, true);
	vaddr = i915_gem_object_pin_map(hwsp->vma->obj, type);
	if (IS_ERR(vaddr)) {
		kfree(cl);
			       ...
			       unsigned int offset)
{
	...
	if (!hwsp) {
		...
		int type = i915_coherent_map_type(gt->i915, hwsp->obj, true);

		vaddr = i915_gem_object_pin_map(hwsp->obj, type);
		if (IS_ERR(vaddr))
